...
const config = {
  ...
  // JWT
  jwt: {
    secret: requireEnv('JWT_SECRET', 'JWT secret key'),
  ...
  // Session
  session: {
    secret: getEnv('SESSION_SECRET', requireEnv('JWT_SECRET', 'Session secret')),
    maxAge: getIntEnv('SESSION_MAX_AGE', 86400000) // 24 hours
  },
  
  // Spotify API
  spotify: {
    clientId: requireEnv('SPOTIFY_CLIENT_ID', 'Spotify client ID'),
    clientSecret: requireEnv('SPOTIFY_CLIENT_SECRET', 'Spotify client secret'),
...
function validateConfig() {
  ...
  if (!config.spotify.clientId || config.spotify.clientId === 'your-client-id') {
    errors.push('SPOTIFY_CLIENT_ID must be set to a valid Spotify client ID');
  }
  
  if (!config.spotify.clientSecret || config.spotify.clientSecret === 'your-client-secret') {
